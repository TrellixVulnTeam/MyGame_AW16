...
my_frame4 = Frame(my_notebook, width=500, height=500)  # onglet 'Réserve pfs'
my_frame4.pack(fill='both', expand=1)
my_frame5 = Frame(my_notebook, width=500, height=500)  # onglet 'Blacklist'
...
my_notebook.add(my_frame4, text='Réserve pfs')
my_notebook.add(my_frame5, text='Blacklist')
...
class Menus(Menu):
    ...
    def message(self, *args):
        ...
        version = messagebox.showinfo('À propos', "FOE - GMs version 5.0\n\n2021 - Laurent REYNAUD")
...
class Reserve(Frame):
    """Les pfs reçus mais non investis dans nos propres Gms"""
    ...
    def calculate(self, *args):
        """Fonction permettant de mettre à jour le nombre de pfs en réserve"""

        """Calcul du nombre de pfs restant en réserve"""
        try:
            my_reserve = self.entry_reserve.get()
            my_addition = self.entry_addition.get()
            my_invest = self.entry_invest.get()
            if my_addition == '':
                my_reserve = int(my_reserve) - int(my_invest)
            elif my_invest == '':
                my_reserve = int(my_reserve) + int(my_addition)
            else:
                my_reserve = int(my_reserve) + int(my_addition) - int(my_invest)
            self.entry_reserve.delete(0, END)
            self.entry_reserve.insert(0, my_reserve)
            self.entry_addition.delete(0, END)
            self.entry_invest.delete(0, END)
        except ValueError:
            pass

        """Assignation de l'obtention des données saisies dans la zone de texte"""
        my_pfs = self.entry_reserve.get()
        ...
        """Ajout du nombre de pfs en réserve dans le fichier en mode écriture (variable output_file)"""
        pickle.dump(my_pfs, output_file)

    def widgets(self):
        """Configuration des wigdets"""

        """Chargement automatique du fichier 'my_pfs'"""
        try:
            file_name = 'my_pfs'  # assignation d'un nom de fichier
            input_file = open(file_name, 'rb')  # assignation de l'ouverture du fichier en mode lecture
            my_pfs = pickle.load(input_file)  # assignation de la récupération des données sauvegardées
        except FileNotFoundError:
            pass

        """Titre de l'onglet"""
        label_title = Label(self, text='Pfs en réserve à investir sur mes Gms', font='helvetica 10 italic')
        label_title.grid(row=0, column=0, columnspan=2, pady=10)

        """Pfs en réserve"""
        label_reserve = Label(self, text='Nombre de pfs en réserve')
        label_reserve.grid(row=1, column=0, padx=5, pady=5)
        self.entry_reserve = Entry(self, justify='center')
        self.entry_reserve.insert(0, my_pfs)  # récupération du nombre de pfs en réserve sauvegardé
        self.entry_reserve.grid(row=1, column=1, padx=5, pady=5)

        """Rajout du nombre de pfs en réserve"""
        label_addition = Label(self, text='Rajout de pfs en réserve')
        label_addition.grid(row=2, column=0, padx=5, pady=5)
        self.entry_addition = Entry(self, justify='center')
        self.entry_addition.grid(row=2, column=1, padx=5, pady=5)

        """Pfs investis"""
        label_invest = Label(self, text='Nombre de pfs investis')
        label_invest.grid(row=3, column=0, padx=5, pady=5)
        self.entry_invest = Entry(self, justify='center')
        self.entry_invest.grid(row=3, column=1, padx=5, pady=5)

        """Bouton permettant de valider les pfs investis et de mettre à jour le nombre de pfs en réserve"""
        btn_validate = Button(self, text='Mise à jour du nombre de pfs en réserve', width=30, command=self.calculate)
        btn_validate.grid(row=4, column=0, columnspan=2, padx=5, pady=20)
...
reserve = Reserve(my_frame4)  # onglet sur la réserve pfs à utiliser
note = Note(my_frame5)  # onglet Blacklist
